				...
				for(var i=0; i<listContainerArray.length; i++)	{
					...
					if(lists.length == 0)	{						
						if($('sidebar'+listType+'UL'))	{
							$('sidebar'+listType+'UL').innerHTML = "No lists currently saved";
					    }
						continue;
					}
	...
	'createOnClicks' : function(theId)	{
		...
		for(var i=0; i<lis.length; i++)	{
			...
			lis[i].onmouseover = function() { 				
				...
				this.title = "";				
				return overlib(tmpp[this.id].split(",").join("<br/>"), CAPTION, this.id + " Elements:", HAUTO, VAUTO, DELAY, 500);
			};
